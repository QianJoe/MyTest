...
#import "UIBarButtonItem+JQBarBtn.h"
#import <MJExtension/MJExtension.h>

@interface JQLocaViewController () <UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,UISearchDisplayDelegate>
...
@implementation JQLocaViewController
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if(tableView == self.searchDisplayController.searchResultsTableView){
        ...
        JQCitiesGroup *group = self.groups[section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView == self.searchDisplayController.searchResultsTableView){
        ...
        JQCitiesGroup *group = self.groups[indexPath.section];
...
{
    JQLocaHeaderView *header = [JQLocaHeaderView headerWithTableView:tableView];
...
-(void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    for (int i = 0; i < self.groups.count; i++) {
        JQCitiesGroup *group = self.groups[i];
...
- (NSMutableArray *)groups
{
    if (_groups == nil) {
        ...
        for (NSDictionary *dict in dictArray) {
            JQCitiesGroup *group = [JQCitiesGroup mj_objectWithKeyValues:dict];
